...
with open('computer_paths.txt','r') as f:
    lines= f.readlines()
    for each_line in lines:
        if 'SI206' in each_line.strip() and '.py' in each_line.strip():
...
with open('computer_paths.txt','r') as f:
    lines= f.readlines()
    for each_line in lines:
        if '.docx' in each_line.strip() or '.xlsx' in each_line.strip():
            if each_line.strip()[-6].isdigit():
...
if __name__ == "__main__":
    ...
    # parse_counted_words('5 watermelons, 13 pineapples, and 1 papaya.')
